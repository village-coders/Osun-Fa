        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
                <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-4 justify-between bg-gray-50/50">
                    ...
                    <span className="text-sm text-gray-400 self-center">{filteredPlayers.length} player{filteredPlayers.length !== 1 ? 's' : ''} found</span>
                </div>
                ...
                <div className="p-6">
                    {filteredPlayers.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <p className="text-gray-500 font-semibold">No players matched your search</p>
                        </div>
                    ) : (
                            ...
                            {paginatedPlayers.map((player) => (
                                <div key={player._id} className="bg-gray-50 border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg hover:border-primary/20 transition-all group">
                                    ...
                                    <div className="relative bg-gradient-to-br from-primary/10 to-primary/5 p-5 flex flex-col items-center text-center">
                                        ...
                                        <h3 className="font-extrabold text-gray-800 text-sm leading-tight">{player.playerName || `${player.surname} ${player.firstName}`}</h3>
                                        <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1">{player.dateOfBirth || 'DOB N/A'}</p>
                                        <span className={`mt-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${player.status === 'Approved' ? 'bg-green-100 text-green-700 border border-green-200' : player.status === 'Pending' ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
                                            {player.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="p-4 space-y-2 text-sm">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="text-gray-700 font-semibold text-xs truncate max-w-[120px]">{player.currentClubName || 'Free Agent'}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-gray-400 text-[10px] uppercase font-bold">Position</span>
                                            <span className="text-gray-700 font-semibold text-xs">{player.playingPosition || 'N/A'}</span>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <span className="text-gray-400 text-[10px] uppercase font-bold">Foot</span>
                                            <span className="text-gray-700 font-semibold text-xs capitalize">{player.dominantFoot || 'N/A'}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="px-4 pb-4 flex items-center justify-between gap-2">
                                        ...
                                        {player.status === 'Pending' && (
                                            <>
                                                <button onClick={() => setReviewMode({ id: player._id, status: 'Approved' })} className="p-2 text-green-600 hover:bg-green-50 rounded-xl transition-all border border-green-200" title="Approve">
                                                    <BadgeCheck size={15} />
                                                </button>
                                                <button onClick={() => setReviewMode({ id: player._id, status: 'Rejected' })} className="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-all border border-red-200" title="Reject">
                                                    <XCircle size={15} />
                                                </button>
                                            </>
                                        )}
                                        <button onClick={() => handleDelete(player._id)} className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Delete">
                                            <Trash2 size={15} />
                                        </button>
                                    </div>
                                </div>
